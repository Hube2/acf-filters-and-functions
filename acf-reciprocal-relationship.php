...
/**
 * Reciprocal relationships - forked from
 * https://github.com/Hube2/acf-filters-and-functions/blob/master/acf-reciprocal-relationship.php
 *
 * Removes hardcoded field keys from the function so that multiple
 * reciprocal filters can be used without having multiple copies
 * of the acf_reciprocal_relationship() function
 *
 * Requires PHP 5.3+
 */
...
add_filter('acf/update_value/key=field_5b310e28be9c2', function ($value, $post_id, $field) use ($key_a, $key_b) {
...
add_filter('acf/update_value/key=field_5b3113ceed9c1', function ($value, $post_id, $field) use ($key_a, $key_b) {
    return acf_reciprocal_relationship($value, $post_id, $field, $key_a, $key_b);
}, 10, 5);
...
// Add the filter to the first relationship field
// The key must match $key_a above
add_filter('acf/update_value/key=field_5b339d33cab0a', function ($value, $post_id, $field) use ($key_a, $key_b) {
    return acf_reciprocal_relationship($value, $post_id, $field, $key_a, $key_b);
}, 10, 5);

// Add the filter to the second relationship field
// The key must match $key_b above
add_filter('acf/update_value/key=field_5b339d70a440d', function ($value, $post_id, $field) use ($key_a, $key_b) {
    return acf_reciprocal_relationship($value, $post_id, $field, $key_a, $key_b);
}, 10, 5);
